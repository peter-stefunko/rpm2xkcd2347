...
from ..model import Package, ParsedSbom


class Spdx23Parser(SbomParser):
    def load(self, path: str | Path) -> ParsedSbom:
        with open(path, encoding="utf-8") as f:
            raw = json.load(f)
        packages = self._parse_packages(raw.get("packages", []))
        dependencies = self._parse_dependencies(raw.get("relationships", []), packages)
        duplicates = self._find_duplicates(packages)
        return ParsedSbom(packages=packages, dependencies=dependencies, duplicates=duplicates)

    def _parse_packages(self, raw: list[dict[str, Any]]) -> dict[str, Package]:
    ...
    def _find_duplicates(self, packages: dict[str, Package]) -> dict[str, list[str]]:
